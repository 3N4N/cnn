...
class LayerMaxPooling:
    ...
    def forward(self, input_neurons):
        print(input_neurons.shape)
        num_neurons = input_neurons.shape[0]
        output = np.zeros((num_neurons, *self.dim_output, self.dim_input[-1]))
        output = output.reshape((num_neurons, np.prod(self.dim_output), self.dim_input[-1]))
        for k in range(num_neurons):
            for j in range(self.dim_input[-1]):
                row = 0
                col = 0
                for i in range(np.prod(self.dim_output)):
                    ...
                    if col + self.dim_filters[1] > self.dim_input[1]:
                        col = 0
                        row += self.dim_filters[0]
        output = output.reshape((num_neurons, *self.dim_output, self.dim_input[-1]))
        return output
...
if __name__ == "__main__":
    ...
    layer1 = LayerConvolution(dim_image, 6, np.array((5,5)), 1, 2)
    o1 = layer1.forward(z[:1000])
    layer2 = LayerMaxPooling(o1.shape[1:], np.array((2,2)), 2)
